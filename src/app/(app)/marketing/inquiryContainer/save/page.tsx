...
export default function InquiryContainer() {
  ...
  React.useEffect(() => {
    if (inquiryDetail && form && !numberParam) {
      form.setFieldsValue(detailDTOToForm(inquiryDetail));
    }
  }, [inquiryDetail, form, numberParam]);

  const [inquiry] = useAction(getInquiry, numberParam, {
    onlyActiveDetail: true,
  });
  React.useEffect(() => {
    if (inquiry && form) {
      form.setFieldsValue(dtoToForm(inquiry));
    }
  }, [inquiry, form]);
    ...
    <SaveLayout<InquiryContainerForm>
      ...
      onSubmit={async (val) => {
        ...
        if (err) {
          ...
          router.replace("/marketing/inquiryContainer");
        }
      }}
      onCancel={() => router.replace("/marketing/inquiryContainer")}
      view={viewParam === "1"}
    >
      <Col span={12}>
